...
const arr1 = [];
function getTrial(city) {
  let key = process.env.TRAIL_API_KEY;
  const url = `https://www.hikingproject.com/data/get-trails${city}lat=40.0274&lon=-105.2519&maxDistance=10&${key}`;
  return superagent.get(url)
    .then(trialData => {
      trialData.body.data.forEach(val => {
